...
public:
	...
	void GiveWeapon(Weapon* pWeapon)
	{
		delete weapon;
		weapon = pWeapon;
	}
...
protected:
	MemeFighter(const std::string& _name, int _hp, int _speed, int _power,Weapon* weapon = nullptr)
		:
		attribution({ _hp, _speed, _power }), name(_name), weapon(weapon)
	{
...
{
public:
	MemeFrog(const std::string& name, Weapon* weapon = nullptr)
		:
...
{
public:
	MemeStoner(const std::string& name, Weapon* weapon = nullptr)
		:
		MemeFighter(name, 80, 4, 14, weapon)
	{
...
int main()
{
	std::vector<MemeFighter*> MemeFrogP = {  new MemeFrog("Frog",new Fists())
											,new MemeFrog("Frog2",new Knife())
										    ,new MemeFrog("Frog3",new Bat())};

	std::vector<MemeFighter*> StonerP = { new MemeStoner("Stoner",new Fists())
										 ,new MemeStoner("Stoner2",new Knife())
										 ,new MemeStoner("Stoner3",new Bat())};
	static_cast<MemeFrog*>(MemeFrogP[1])->GetName();

	const auto alive = [](auto other) {return other->isAlive(); };
